...
##stepwise model selection asked by the reviewer
...
#remove helper/non helper
mod_2<-glmer(Feeding_visits~ Playback_type*sex +
               Time_Since_sun_rise.z+
               N_chicks.z+ 
               D_chicks.z+ 
               temperature.z+
...
#remove sex
mod_3<-glmer(Feeding_visits~ Playback_type*class_breeder +
               Time_Since_sun_rise.z+
               N_chicks.z+ 
               D_chicks.z+ 
               temperature.z+
...
#remove playback type experiments
mod_4<-glmer(Feeding_visits~ class_breeder*sex +
               Time_Since_sun_rise.z+
               N_chicks.z+ 
               D_chicks.z+ 
               temperature.z+
...
#remove time since sunrise
mod_5<-glmer(Feeding_visits~ Playback_type*class_breeder*sex +
               N_chicks.z+ 
               D_chicks.z+ 
               temperature.z+
               wind.z+
...
#remove Number of chicks
mod_6<-glmer(Feeding_visits~ Playback_type*class_breeder*sex +
               Time_Since_sun_rise.z+
               D_chicks.z+ 
               temperature.z+
               wind.z+
...
#remove Day of the chicks
mod_7<-glmer(Feeding_visits~ Playback_type*class_breeder*sex +
               Time_Since_sun_rise.z+
               N_chicks.z+ 
               temperature.z+
               wind.z+
...
#remove temperature
mod_8<-glmer(Feeding_visits~ Playback_type*class_breeder*sex +
               Time_Since_sun_rise.z+
               N_chicks.z+ 
               D_chicks.z+
               wind.z+
...
#remove wind
mod_9<-glmer(Feeding_visits~ Playback_type*class_breeder*sex +
               Time_Since_sun_rise.z+
               N_chicks.z+ 
               D_chicks.z+
               temperature.z+
...
#remove Recording_day
mod_10<-glmer(Feeding_visits~ Playback_type*class_breeder*sex +
               Time_Since_sun_rise.z+
               N_chicks.z+ 
               D_chicks.z+
               temperature.z+
...
#remove playback 
mod_11<-glmer(Feeding_visits~ Playback_type*class_breeder*sex +
               Time_Since_sun_rise.z+
               N_chicks.z+ 
               D_chicks.z+
               temperature.z+
...
lrtest(mod_1, mod_2, mod_3, mod_4, mod_5, mod_6, mod_7, mod_8, mod_9, mod_10, mod_11)



mod_min <-glmer(Feeding_visits~ Playback_type*class_breeder*sex +
                N_chicks.z+ 
                D_chicks.z+
                temperature.z+
                wind.z +
...
summary(mod_min)
emmeans(mod_min, pairwise ~Playback_type|sex|class_breeder)
...
### CHECK HOW MANY HELPERS SIRE OFFSPRING #####

#load audience dataset
audience_dataset<-read.csv("C:/Users/pietr/Documents/Sociable_Weavers/Audience_effect/Datasets/audience_dataset.csv")

#list of all the broods
broodsID<-unique(audience_dataset$ColNestLaying)

#list of all the helpers
helpers<-unique(audience_dataset$BirdID_CNL[audience_dataset$class_breeder=="helper"])

#load the genetic dataset
Genetic_Parentage20220718 <- read.csv("~/Sociable_Weavers/genetic_analysis/genotype_dataset/Genetic_Parentage20220718.csv")
#change the format of the date
Genetic_Parentage20220718$laying_date<-format(as.Date(Genetic_Parentage20220718$laying_date, format="%d-%m-%y"),"%Y-%m-%d")

#create the brood ID
Genetic_Parentage20220718$ColNestLaying<- paste(Genetic_Parentage20220718$colony, Genetic_Parentage20220718$nest, Genetic_Parentage20220718$laying_date, sep="_")

#select only the lines concerning the experiment 
Genetic_Parentage20220718_<-Genetic_Parentage20220718[which(Genetic_Parentage20220718$ColNestLaying %in% broodsID),]

#create a unique ID for the bird + the breeding attempt 
Genetic_Parentage20220718_$bird_broodID<- paste(Genetic_Parentage20220718_$father, Genetic_Parentage20220718_$ColNestLaying, sep = "-")

##lines of the dataset that include helpers
Genetic_Parentage20220718_[which(Genetic_Parentage20220718_$bird_broodID %in% helpers),]

# offspring    season colony nest brood egg_num laying_date  father  mother  prob    criterion   ColNestLaying
#   BH42889 2017/2018     37    2     1       I  2017-10-13 BJ09866 BJ09734 0.614 inconclusive 37_2_2017-10-13
# bird_broodID
# BJ09866-37_2_2017-10-13

# only one helper has possibly sired a chick in the brood it was helping  
